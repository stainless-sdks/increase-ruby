    ...
    module Type
      ...
      module Union
        ...
        private def resolve_variant(value)
          ...
          in [_, Increase::Internal::Type::BaseModel]
            value.class
          in [Symbol, Hash]
            key = value.fetch(@discriminator) do
              value.fetch(@discriminator.to_s, Increase::Internal::OMIT)
            end
        ...
        def dump(value, state:)
          if (target = resolve_variant(value))
            return Increase::Internal::Type::Converter.dump(target, value, state: state)
          end
        ...
        def inspect(depth: 0)
          ...
          "#{name}[#{variants.map { Increase::Internal::Type::Converter.inspect(_1, depth: depth.succ) }.join(' | ')}]"
          # rubocop:enable Layout/LineLength
        end
      end
    end
