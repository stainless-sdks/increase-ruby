      ...
      class BaseClient
        ...
        class << self
          ...
          def should_retry?(status, headers:)
            coerced = Increase::Internal::Util.coerce_boolean(headers["x-should-retry"])
            case [coerced, status]
            in [true | false, _]
              coerced
          ...
          def follow_redirect(request, status:, response_headers:)
            ...
            # from undici
            if Increase::Internal::Util.uri_origin(url) != Increase::Internal::Util.uri_origin(location)
          ...
          def reap_connection!(status, stream:)
            ...
            in (..199) | (300..499)
              stream&.each { next }
            in Increase::Errors::APIConnectionError | (500..)
              Increase::Internal::Util.close_fused!(stream)
            else
            end
          end
        end
        ...
        #
        #   @option req [Symbol, Integer, Array<Symbol, Integer>, Proc, nil, nil] :unwrap
        #
        ...
        private def build_request(req, opts)
          ...
          path = Increase::Internal::Util.interpolate_path(uninterpolated_path)
        ...
        private def send_request(request, redirect_count:, retry_count:, send_retry_header:)
          ...
          in (400..) | Increase::Errors::APIConnectionError
            self.class.reap_connection!(status, stream: stream)
        ...
        #
        # @param unwrap [Symbol, Integer, Array<Symbol, Integer>, Proc, nil, nil]
        #
