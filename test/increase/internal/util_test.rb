...
class Increase::Test::UtilDataHandlingTest < Minitest::Test
  def test_left_map
    assert_pattern do
      Increase::Internal::Util.deep_merge({a: 1}, nil) => nil
    end
  end

  def test_right_map
    assert_pattern do
      Increase::Internal::Util.deep_merge(nil, {a: 1}) => {a: 1}
    end
  end

  def test_disjoint_maps
    assert_pattern do
      Increase::Internal::Util.deep_merge({b: 2}, {a: 1}) => {a: 1, b: 2}
    end
  end

  def test_overlapping_maps
    assert_pattern do
      Increase::Internal::Util.deep_merge({b: 2, c: 3}, {a: 1, c: 4}) => {a: 1, b: 2, c: 4}
    end
  end

  def test_nested
    assert_pattern do
      Increase::Internal::Util.deep_merge({b: {b2: 1}}, {b: {b2: 2}}) => {b: {b2: 2}}
    end
  end

  def test_nested_left_map
    assert_pattern do
      Increase::Internal::Util.deep_merge({b: {b2: 1}}, {b: 6}) => {b: 6}
    end
  end

  def test_omission
    merged = Increase::Internal::Util.deep_merge(
      {b: {b2: 1, b3: {c: 4, d: 5}}},
      {b: {b2: 1, b3: {c: Increase::Internal::Util::OMIT, d: 5}}}
    )
  ...
  def test_dig
    assert_pattern do
      Increase::Internal::Util.dig(1, nil) => 1
      Increase::Internal::Util.dig({a: 1}, :b) => nil
      Increase::Internal::Util.dig({a: 1}, :a) => 1
      Increase::Internal::Util.dig({a: {b: 1}}, [:a, :b]) => 1

      Increase::Internal::Util.dig([], 1) => nil
      Increase::Internal::Util.dig([nil, [nil, 1]], [1, 1]) => 1
      Increase::Internal::Util.dig({a: [nil, 1]}, [:a, 1]) => 1
...
class Increase::Test::UtilUriHandlingTest < Minitest::Test
  def test_parsing
    %w[
      ...
      parsed = Increase::Internal::Util.parse_uri(url)
      unparsed = Increase::Internal::Util.unparse_uri(parsed).to_s

      assert_equal(url, unparsed)
      assert_equal(parsed, Increase::Internal::Util.parse_uri(unparsed))
    end
  end
  ...
  def test_joining
    cases = [
      [
        "h://a.b/c?d=e",
        "h://nope/ignored",
        Increase::Internal::Util.parse_uri("h://a.b/c?d=e")
      ],
      [
        "h://a.b/c?d=e",
        "h://nope",
    ...
    cases.each do |expect, lhs, rhs|
      assert_equal(
        URI.parse(expect),
        Increase::Internal::Util.join_parsed_uri(
          Increase::Internal::Util.parse_uri(lhs),
          rhs
        )
      )
    end
  ...
  def test_joining_queries
    ...
    cases.each do |path, expected|
      assert_equal(
        URI.parse(expected),
        Increase::Internal::Util.join_parsed_uri(
          Increase::Internal::Util.parse_uri(base_url),
          {path: path}
        )
      )
    end
...
class Increase::Test::UtilIOAdapterTest < Minitest::Test
  def test_copy_read
    ...
    cases.each do |input, expected|
      io = StringIO.new
      # rubocop:disable Lint/EmptyBlock
      adapter = Increase::Internal::Util::ReadIOAdapter.new(input) {}
      # rubocop:enable Lint/EmptyBlock
  ...
  def test_copy_write
    ...
    cases.each do |input, expected|
      enum = Increase::Internal::Util.writable_enum do |y|
...
class Increase::Test::UtilFusedEnumTest < Minitest::Test
  def test_closing
    arr = [1, 2, 3]
    once = 0
    fused = Increase::Internal::Util.fused_enum(arr.to_enum) do
      once = once.succ
    end
  ...
  def test_external_iteration
    it = [1, 2, 3].to_enum
    first = it.next
    fused = Increase::Internal::Util.fused_enum(it, external: true)
  ...
  def test_closed_fused_extern_iteration
    ...
    fused = Increase::Internal::Util.fused_enum(enum)
    first = fused.next
  ...
  def test_close_fused_sse_chain
    ...
    fused_1 = Increase::Internal::Util.fused_enum(enum)
    fused_2 = Increase::Internal::Util.decode_lines(fused_1)
    fused_3 = Increase::Internal::Util.decode_sse(fused_2)

    assert_equal(0, taken)
    Increase::Internal::Util.close_fused!(fused_3)
    assert_equal(0, taken)
  end
end
...
class Increase::Test::UtilSseTest < Minitest::Test
  def test_decode_lines
    ...
    cases.each do |enum, expected|
      eols.each do |eol|
        lines = Increase::Internal::Util.decode_lines(enum.map { _1.gsub("\n", eol) })
  ...
  def test_mixed_decode_lines
    ...
    cases.each do |enum, expected|
      lines = Increase::Internal::Util.decode_lines(enum)
  ...
  def test_decode_sse
    ...
    cases.each do |name, test_cases|
      test_cases.each do |input, expected|
        actual = Increase::Internal::Util.decode_sse(input).map(&:compact)
